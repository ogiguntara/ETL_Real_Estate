...
#Source path where we want to save the .zip file downloaded from website
source_path=f"{base_path}/data/source/downloaded_at={now}/PPR-ALL.zip"

#Raw path where we want to extract the new .csv
raw_path=f"{base_path}/data/raw/downloaded_at={now}/ppr-all.csv"
    ...
    with tempfile.TemporaryDirectory() as dirpath:
        with ZipFile(source_path,mode="r") as zipfile:
            ...
            with open(csv_file_path,mode='r',encoding='windows-1252') as csv_file:
                ...
                row = next(reader)#get first row on reader
